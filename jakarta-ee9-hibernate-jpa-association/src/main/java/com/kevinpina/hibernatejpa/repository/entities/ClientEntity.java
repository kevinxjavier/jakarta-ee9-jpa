...
public class ClientEntity {
	...
	@OneToMany(cascade = CascadeType.ALL, orphanRemoval = true) // One=.class To Many=attribute (One Client has Many Addresses )
	...
	// will create the same as lines 53 but personalized @JoinColumn must be commented
	// joinColumns main foreign key
	// inverseJoinColumns secondary foreign key
	@JoinTable(name = "tbl_client_address", joinColumns = @JoinColumn(name = "id_client"), inverseJoinColumns = @JoinColumn(name = "id_address"), uniqueConstraints = @UniqueConstraint(columnNames = {"id_address"}))
	...
	// mappedBy: for a relationship bidirectional only in the @OnetoMany we have to indicate the inverse relationship with this property "mappedBy",
	// 		putting the attribute defined in InvoiceEntity that point to this class that is in this case clientEntity.
	@OneToMany(cascade = CascadeType.ALL, orphanRemoval = true, mappedBy = "clientEntity")
